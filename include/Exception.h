...
public:
    ...
    /** 构造函数 */
    Exception(const std::string &file, int line, Grade::Type grade, int code, const std::string &msg);

    /** 构造函数 */
    template < typename ECAUSE >
    Exception(const std::string &file, int line, const ECAUSE &cause, Grade::Type grade, int code, const std::string &msg);
    
    /** 构造函数 */
    Exception(const std::string &file, int line, Grade::Type grade, int code, const char *msg, ...);

    /** 构造函数 */
    template < typename ECAUSE >
    Exception(const std::string &file, int line, const ECAUSE &cause, Grade::Type grade, int code, const char *msg, ...);
...
protected:
    ...
    /**
     * 引起此异常的根异常, 比如一个基于文件的数据库抛出SQLException, 它的根异常有可能是IOException
     */
